...
async function getTodos() {
  try {
    ...
    const dataData = data.data.continents
    console.log(dataData);
    for (const continent of dataData) {
      const optionElement = document.createElement("option");
      optionElement.value = continent.name;
      optionElement.textContent = continent.name;
      selectElement.appendChild(optionElement);
    }
    selectElement.addEventListener("change", (event) => {
      const selectedContinent = dataData.find(continent => continent.name === event.target.value);
      if (selectedContinent) {
        const countries = selectedContinent.countries.map(country => country.name);
        results.textContent = `Countries: ${countries.join(", ")}`;
      } else {
        results.textContent = "";
      }
    })
